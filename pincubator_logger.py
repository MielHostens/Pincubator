...
def write_history():
	write_latest_th(pin1, latest_temperature1, latest_humidity1) 
	write_latest_th(pin2, latest_temperature2, latest_humidity2)
...
try:
  while True:
    ...
    if hum is not None and temp is not None:
       write_latest_th(pin1, temp, hum)
       latest_temperature1 = temp
       latest_humidity1 = hum
    hum, temp = Adafruit_DHT.read_retry(sensor, pin2)
    if hum is not None and temp is not None:
       write_latest_th(pin2, temp, hum)
       latest_temperature2 = temp
       latest_humidity2 = hum
    time.sleep(1)
